...
# --- "World-Class" Fix: Force IPv4 and set reasonable timeout ---
# This prevents hangs and connection failures caused by IPv6 routing issues
# which are common with Gmail/Office365 on many ISP networks.
...
def run_script(script_name, args=None):
    ...
    # --- "World-Class" Pre-flight Check ---
    if not os.path.exists(script_path):
        print(f"\n[red]ERROR: Script file not found: '{script_path}'[/red]")
        print("[yellow]This might be a missing file from the project. Please ensure all files are present.[/yellow]")
...
def _update_env_passwords(email, password):
    ...
    if not current_passwords_str: current_passwords_str = "{}"

    # --- "World-Class" Robustness: Handle malformed JSON in .env ---
    try:
        passwords = json.loads(current_passwords_str)
    except json.JSONDecodeError:
        import re
        # Attempt to repair trailing commas which are common in manual edits
        try:
            passwords = json.loads(re.sub(r',(\s*})', r'\1', current_passwords_str))
        except Exception:
            print("  -> [yellow]Warning: Could not parse SMTP_PASSWORDS in .env. Resetting storage.[/yellow]")
            passwords = {}
...
def guided_setup():
    ...
    if enable_smime == 'y':
        # Ensure the section exists before trying to set values in it
    ...
    # 4. AI Content Engine
    print("\n[4] AI Content Engine (for unique content generation & reply analysis)")
    current_ai = cfg.getboolean('AI', 'ai_enabled', fallback=False)
    enable_ai = input(f"Enable AI features? [y/n] (current: {'Yes' if current_ai else 'No'}): ").strip().lower()
    if enable_ai == 'y':
        if not cfg.has_section('AI'):
            cfg.add_section('AI')
        cfg.set('AI', 'ai_enabled', 'true')
        ...
        if env_path and dotenv_values:
            ...
            api_key_set = "GROQ_API_KEY" in env_vars and env_vars["GROQ_API_KEY"]
        
        if not api_key_set:
            print("[yellow]WARNING: AI is enabled, but your Groq API key is not set.[/yellow]")
            print("   Please go to 'Configuration & Setup' -> 'AI Content Engine' -> 'Manage API Keys' to add it.")
...
def _select_ai_provider(cfg):
    """A helper menu to select the AI provider and model."""
    console = Console()
    # "Smarter" Provider List with explanations
    providers = {
        '1': {'name': 'groq', 'models': ['llama-3.1-8b-instant', 'llama-3.1-70b-versatile', 'mixtral-8x7b-32768'], 'desc': "Cloud (Fast). Generous free tier, but has limits."},
        '2': {'name': 'ollama', 'models': ['llama3', 'codegemma', 'mixtral'], 'desc': "Local (Private). [bold green]UNLIMITED[/bold green] Usage (No Quotas). Requires Ollama app."},
        '3': {'name': 'gemini', 'models': ['gemini-2.0-flash', 'gemini-2.0-flash-lite', 'gemini-2.5-flash', 'gemini-2.5-pro', 'gemini-1.5-flash'], 'desc': "Google Cloud. Smartest. Free tier has rate limits (Quota)."}
    }
    console.print("\n--- Select AI Provider ---")
    console.print("1) Groq ([bold green]Fastest[/bold green], Cloud, Free API) - [bold cyan]Recommended for Speed[/bold cyan]")
    console.print("2) Ollama ([bold yellow]Private[/bold yellow], Local, Slower)")
    console.print("3) Gemini ([bold blue]Smartest[/bold blue], Cloud, Free API)")
    ...
    if provider_choice in providers:
        ...
        cfg.set('AI', 'ai_provider', provider_name)

        console.print(f"\n--- Select Model for {provider_name.upper()} ---")
        if provider_name == 'ollama':
            # --- "World-Class" Auto-Discovery for Ollama Models ---
            found_models = []
            try:
                import requests
                ...
                if resp.status_code == 200:
                    data = resp.json()
                    found_models = [m['name'] for m in data.get('models', [])]
            except Exception:
                pass

            if found_models:
                console.print(f"[green]Detected installed Ollama models:[/green]")
                for i, m in enumerate(found_models, 1):
                    console.print(f"{i}) {m}")
                console.print(f"{len(found_models) + 1}) [Manual Input]")

            console.print("\nEnter the name of the Ollama model (e.g., 'llama3', 'mistral') or select from above.")
            console.print("Press Enter to keep the current model.")
            current_model = cfg.get('AI', 'ai_model', fallback='llama3')
            model_name = input(f"Model name (current: {current_model}): ").strip()
            if model_name:
                # Check if user selected a number from the detected list
                if model_name.isdigit() and found_models:
                    idx = int(model_name) - 1
                    if 0 <= idx < len(found_models):
                        model_name = found_models[idx]
                
                cfg.set('AI', 'ai_model', model_name)
                console.print(f"[green]Provider set to 'ollama' with model '{model_name}'.[/green]")
            else:
                console.print(f"[green]Provider set to 'ollama', model remains '{current_model}'.[/green]")
        else: # Groq
            ...
            if model_choice.isdigit():
                try:
                    ...
                    if 0 <= model_index < len(selected_provider['models']):
                        selected_model = selected_provider['models'][model_index]
                except IndexError:
                    pass
            else:
            ...
            if selected_model:
                cfg.set('AI', 'ai_model', selected_model)
                console.print(f"[green]Provider set to '{provider_name}' with model '{selected_model}'.[/green]")
            else:
...
def ai_config_menu():
    ...
    while True:
        ...
        current_provider = cfg.get('AI', 'ai_provider', fallback='groq').upper()
        current_model = cfg.get('AI', 'ai_model', fallback='llama-3.1-8b-instant')
        ...
        rewrite_body = cfg.getboolean('AI', 'ai_rewrite_body', fallback=False)
        ...
        console.print(f"2) Select Provider & Model (Currently: [bold cyan]{current_provider}[/bold cyan] - {current_model})")
        console.print(f"3) Manage API Keys (Groq Key Status: {'[green]SET[/green]' if api_key_set else '[red]NOT SET[/red]'})")
        console.print(f"4) Toggle Subject Rewriting (Currently: {'[green]ON[/green]' if rewrite_subject else '[yellow]OFF[/yellow]'})")
        console.print(f"5) Toggle IMAP Reply Classifier (Currently: {'[green]ON[/green]' if classify_replies else '[yellow]OFF[/yellow]'})")
        console.print(f"6) Toggle Intro Sentence Generation (Currently: {'[green]ON[/green]' if generate_intro else '[yellow]OFF[/yellow]'})")
        console.print(f"7) Toggle Body Rewriting [bold cyan](Polymorphic Content)[/bold cyan] (Currently: {'[green]ON[/green]' if rewrite_body else '[yellow]OFF[/yellow]'})")
        ...
        if choice == '1':
            ...
            cfg.set('AI', 'ai_rewrite_body', str(int(not rewrite_body)))
        elif choice == '0':
            break
        else:
...
def api_key_menu():
    ...
    while True:
        ...
        if env_path and dotenv_values:
            ...
            groq_key_set = "GROQ_API_KEY" in env_vars and env_vars["GROQ_API_KEY"]
            gemini_key_set = "GEMINI_API_KEY" in env_vars and env_vars["GEMINI_API_KEY"]

        console.print("\n--- [bold]API Key Management[/bold] ---")
        console.print(f"1) Set Groq API Key (Status: {'[green]SET[/green]' if groq_key_set else '[red]NOT SET[/red]'}) [bold yellow](Free & Fast!)[/bold yellow]")
        console.print(f"2) Set Hunter.io API Key (Status: {'[green]SET[/green]' if hunter_key_set else '[red]NOT SET[/red]'})")
        console.print(f"3) Set Gemini API Key (Status: {'[green]SET[/green]' if gemini_key_set else '[red]NOT SET[/red]'})")
        console.print("0) Back")
        choice = input("Enter choice: ").strip()

        if choice == '1':
            key = input("Enter your Groq API Key: ").strip()
            if key: set_key(env_path, "GROQ_API_KEY", key)
        elif choice == '2':
            ...
            if key: set_key(env_path, "GEMINI_API_KEY", key)
        elif choice == '0':
            break

def sentinel_config_menu():
    """Menu to configure Sentinel Mark X settings."""
    console = Console()
    cfg = configparser.ConfigParser(inline_comment_prefixes=('#',))
    cfg.read(CONFIG_PATH)
    
    if not cfg.has_section('SENTINEL'):
        cfg.add_section('SENTINEL')

    while True:
        console.print("\n--- [bold red]Sentinel Mark X Configuration[/bold red] ---")
        
        enabled = cfg.getboolean('SENTINEL', 'sentinel_enabled', fallback=False)
        strict = cfg.getboolean('SENTINEL', 'sentinel_strict_mode', fallback=False)
        poly = cfg.getboolean('SENTINEL', 'sentinel_polymorphic_mode', fallback=True)
        adaptive = cfg.getboolean('SENTINEL', 'sentinel_adaptive_delivery', fallback=True)
        threshold = cfg.getfloat('SENTINEL', 'sentinel_trigger_threshold', fallback=0.15)
        strict_threshold = cfg.getfloat('SENTINEL', 'sentinel_strict_threshold', fallback=0.50)

        status_color = "green" if enabled else "red"
        
        console.print(f"1) Toggle Sentinel Engine (Currently: [{status_color}]{'ENABLED' if enabled else 'DISABLED'}[/{status_color}])")
        console.print(f"2) Toggle Strict Mode [bold red](Annihilation Protocol)[/bold red] (Currently: {'[green]ON[/green]' if strict else '[yellow]OFF[/yellow]'})")
        console.print(f"3) Toggle Polymorphic Shield (Auto-AI Body) (Currently: {'[green]ON[/green]' if poly else '[yellow]OFF[/yellow]'})")
        console.print(f"4) Toggle Adaptive Delivery (Link Rotation) (Currently: {'[green]ON[/green]' if adaptive else '[yellow]OFF[/yellow]'})")
        console.print(f"5) Set Defense Trigger Threshold (Currently: {int(threshold*100)}% failure rate)")
        console.print(f"6) Set Strict Mode Threshold (Currently: {int(strict_threshold*100)}% failure rate)")
        console.print("0) Back")
        
        choice = input("Enter choice: ").strip()
        
        if choice == '1':
            cfg.set('SENTINEL', 'sentinel_enabled', '0' if enabled else '1')
        elif choice == '2':
            cfg.set('SENTINEL', 'sentinel_strict_mode', '0' if strict else '1')
        elif choice == '3':
            cfg.set('SENTINEL', 'sentinel_polymorphic_mode', '0' if poly else '1')
        elif choice == '4':
            cfg.set('SENTINEL', 'sentinel_adaptive_delivery', '0' if adaptive else '1')
        elif choice == '5':
            try:
                new_val = float(input("Enter new trigger threshold (0.01 to 1.0, e.g., 0.15 for 15%): ").strip())
                cfg.set('SENTINEL', 'sentinel_trigger_threshold', str(new_val))
            except ValueError:
                console.print("[red]Invalid number.[/red]")
        elif choice == '6':
            try:
                new_val = float(input("Enter new strict threshold (0.01 to 1.0, e.g., 0.50 for 50%): ").strip())
                cfg.set('SENTINEL', 'sentinel_strict_threshold', str(new_val))
            except ValueError:
                console.print("[red]Invalid number.[/red]")
        elif choice == '0':
            break
        ...
        with open(CONFIG_PATH, 'w') as f:
            cfg.write(f)
        console.print("[green]Sentinel settings updated.[/green]")
...
def eml_settings_menu():
    ...
    while True:
        ...
        with open(CONFIG_PATH, 'w') as f:
            cfg.write(f)
        console.print("[green]Success![/green] EML settings updated.")
    ...
    while True:
        ...
        if choice == '1':
            ...
            if sub == 'a':
                ...
                try:
                    from function.smtp_utils import discover_smtp_settings, test_smtp_connection
                    import logging
                    ...
                    for cred in new_creds:
                        if ':' not in cred:
                            print(f"  -> [red]Invalid format for '{cred}'. Skipping.[/red]")
                            continue
                        email, password = cred.split(':', 1)
                        
                        # Call the new discover function which returns (config, error)
                        config_dict, error_msg = discover_smtp_settings(email.strip(), password.strip(), logger=test_logger)
                        
                        if config_dict:
                            # Success! The discover function already tested it.
                            print(f"  -> [green]Success![/green] Discovered and verified settings for {email}.")
                            _update_env_passwords(email.strip(), password.strip())
                            # --- "World-Class" Security Fix: Do NOT write the password to config.ini ---
                            # Match the manual add format: host|port|email|password|security
                            new_entry = f"{config_dict['host']}|{config_dict['port']}|{config_dict['email']}||{config_dict['security']}"
                            entries.append(new_entry)
                            added_count += 1
                        else:
                            # Failure, print the helpful error message.
            ...
            # We need to ensure the PROXY section exists
            if not cfg.has_section('PROXY'): cfg.add_section('PROXY')
            while True:
                print('\n--- Proxy Settings ---')
                proxy_enabled = cfg.getboolean('PROXY', 'proxy_enabled', fallback=False)
                proxy_ai = cfg.getboolean('PROXY', 'proxy_ai_connections', fallback=False)
                status = "ENABLED" if proxy_enabled else "DISABLED"
                ai_status = "ENABLED" if proxy_ai else "DISABLED"
                print(f"1) Toggle Proxies for SMTP (Currently: {status})")
                print(f"2) Toggle Proxies for AI (Currently: {ai_status})")
                print("3) Edit Proxy List (manual)")
                print("4) [bold cyan]Setup Local Dichvusocks App[/bold cyan]")
                print("5) [dim]Set Premium Proxy Site Credentials (for website scraping)[/dim]")
                print("6) [bold cyan]Test All Configured Proxies[/bold cyan]")
                ...
                if proxy_choice == '1':
                    ...
                    with open(CONFIG_PATH, 'w') as f:
                        cfg.write(f)
                    print(f"-> SMTP Proxies are now {'ENABLED' if new_status else 'DISABLED'}.")
                elif proxy_choice == '2':
                    new_status = not proxy_ai
                    cfg.set('PROXY', 'proxy_ai_connections', '1' if new_status else '0')
                    with open(CONFIG_PATH, 'w') as f:
                        cfg.write(f)
                    print(f"-> AI Proxies are now {'ENABLED' if new_status else 'DISABLED'}.")
                elif proxy_choice == '3':
                    ...
                    run_script('setup_local_dichvu_proxies.py')
                elif proxy_choice == '5':
                    console.print("\n--- [bold]Set Premium Proxy Site Credentials[/bold] ---")
                    console.print("Used by scripts like 'fetch_dichvusocks_proxies.py' to log in.")
                    current_user = cfg.get('PROXY', 'proxy_site_username', fallback='')
                    current_port = cfg.get('PROXY', 'proxy_site_port', fallback='')
                    site_user = input(f"Enter site username (current: {current_user}): ").strip()
                    site_pass = input(f"Enter site password (leave blank to keep current): ").strip()
                    site_port = input(f"Enter site port (the port for all proxies, e.g. 1080) (current: {current_port}): ").strip()
                    if site_user:
                        cfg.set('PROXY', 'proxy_site_username', site_user)
                    if site_pass:
                        cfg.set('PROXY', 'proxy_site_password', site_pass)
                    if site_port:
                        cfg.set('PROXY', 'proxy_site_port', site_port)
                    with open(CONFIG_PATH, 'w') as f:
                        cfg.write(f)
                    console.print("[green]-> Credentials updated in config.ini.[/green]")
                elif proxy_choice == '6':
                    run_script('proxy_connectivity_test.py')
                elif proxy_choice == '0':
                    break
                else:
...
def ews_config_menu():
    """A dedicated menu for configuring EWS, especially for GoDaddy-federated accounts."""
    console = Console()
    cfg = configparser.ConfigParser(inline_comment_prefixes=('#',))
    cfg.read(CONFIG_PATH)

    if not cfg.has_section('EWS'):
        cfg.add_section('EWS')

    while True:
        console.print("\n--- [bold]EWS (Office365 / GoDaddy) Setup[/bold] ---")
        console.print("GoDaddy-managed Microsoft accounts often require a special login method.")
        console.print("[yellow]The recommended method is using Browser Cookies.[/yellow]")
        console.print("[bold red]IMPORTANT: For Office 365 SMTP, ensure 'Authenticated SMTP' is ENABLED for the user in the Microsoft 365 Admin Center.[/bold red]")
        console.print("[dim]If cookies fail, the app can fall back to a password if one is saved via the SMTP 'Smart Add' menu.[/dim]")

        username = cfg.get('EWS', 'ews_username', fallback='Not Set')
        has_cookies = bool(cfg.get('EWS', 'ews_cookies', fallback=''))
        sending_method = cfg.get('GENERAL', 'sending_method', fallback='smtp').lower()
        
        # --- Check for Backup Password (Hybrid Mode) ---
        has_password = False
        try:
            if find_dotenv and dotenv_values:
                env_path = find_dotenv()
                if env_path:
                    env_vals = dotenv_values(env_path)
                    raw_passwords = env_vals.get("SMTP_PASSWORDS", "{}")
                    pass_dict = json.loads(raw_passwords)
                    if pass_dict.get(username):
                        has_password = True
        except Exception:
            pass

        console.print(f"Current Sending Protocol: [bold {'green' if sending_method == 'ews' else 'yellow'}]{sending_method.upper()}[/]")
        console.print(f"\n1) Set EWS Username (Currently: [cyan]{username}[/cyan])")
        console.print(f"2) Paste Browser Cookies (Status: {'[green]SET[/green]' if has_cookies else '[red]NOT SET[/red]'})")
        console.print(f"   [dim]Backup Password Status:[/] {'[green]SET (Hybrid Mode Active)[/green]' if has_password else '[yellow]NOT SET (Session may expire)[/yellow]'}")
        console.print("3) [dim]Advanced: Toggle to OAuth/Basic Auth[/dim]")
        console.print(f"4) [bold]Toggle Protocol: Switch to {'SMTP' if sending_method == 'ews' else 'EWS'}[/bold]")
        console.print(f"5) Clear Saved Cookies (Force Basic/Password Auth)")
        console.print(f"6) [bold green]Set Backup Password[/bold green] (Fixes 'No password found' error)")
        console.print("0) Back to Configuration Menu")

        choice = input("Enter choice: ").strip()

        if choice == '1':
            new_username = input(f"Enter EWS Username (e.g., admin@yourdomain.com): ").strip()
            if new_username:
                cfg.set('EWS', 'ews_username', new_username)
        elif choice == '2':
            console.print("\n--- [bold]Paste Browser Cookies[/bold] ---")
            console.print("1. In Chrome/Edge, log in to [bold cyan]outlook.office.com[/bold cyan].")
            console.print("2. Install the 'EditThisCookie' browser extension from the web store.")
            console.print("3. With the Outlook tab active, click the extension icon and then the 'Export' button.")
            console.print("4. Paste the copied text below. [bold yellow]Press Enter on a blank line to finish.[/bold yellow]")
            ...
            # If the user pastes a JS snippet (like the one you provided), extract the JSON array.
            final_cookies = raw_cookies
            if raw_cookies and not raw_cookies.startswith('['):
                s = raw_cookies.find('[')
                e = raw_cookies.rfind(']')
                if s != -1 and e != -1 and e > s:
                    candidate = raw_cookies[s:e+1]
                    # Verify it looks like JSON
                    try:
                        json.loads(candidate)
                        final_cookies = candidate
                        console.print("[yellow]Detected code wrapper. Automatically extracted the JSON array.[/yellow]")
                    except Exception:
                        pass

            if final_cookies:
                # Validate JSON before saving to prevent runtime crashes
                try:
                    json.loads(final_cookies)
                    cfg.set('EWS', 'ews_cookies', final_cookies)
                    cfg.set('EWS', 'ews_use_oauth', 'false') # Ensure cookie method is active
                    console.print("[green]Cookies saved successfully.[/green]")
                except json.JSONDecodeError:
                    console.print("[red]Invalid JSON detected.[/red]")
                    console.print(f"Input received: {final_cookies[:50]}...")
                    console.print("Please ensure you copy the list starting with '[' and ending with ']'.")
            else:
                console.print("[yellow]No input received. Cookies not saved.[/yellow]")
        elif choice == '3':
            use_oauth = cfg.getboolean('EWS', 'ews_use_oauth', fallback=False)
            cfg.set('EWS', 'ews_use_oauth', str(not use_oauth).lower())
            if not use_oauth:
                console.print("[yellow]Switched to OAuth mode. The app will attempt an interactive login on next run.[/yellow]")
            else:
                console.print("[yellow]Switched to Basic/Cookie auth mode. Ensure cookies are set or a password is in .env.[/yellow]")
        elif choice == '4':
            new_method = 'smtp' if sending_method == 'ews' else 'ews'
            cfg.set('GENERAL', 'sending_method', new_method)
            console.print(f"[green]Sending method switched to {new_method.upper()}.[/green]")
        elif choice == '5':
            cfg.set('EWS', 'ews_cookies', '')
            with open(CONFIG_PATH, 'w') as f:
                cfg.write(f)
            console.print("[green]Cookies cleared. EWS will now try to use Basic Auth (Password) if configured.[/green]")
        elif choice == '6':
            console.print(f"\nEnter the password for [cyan]{username}[/cyan].")
            p = input("Password: ").strip()
            if p: _update_env_passwords(username, p)
        elif choice == '0':
            break
...
def config_menu():
    ...
    while True:
        ...
        menu_table.add_row('8)', '[bold red]Toggle Ghost Sending (Delete Sent Items)[/bold red]')
        menu_table.add_row('9)', '[bold]EWS (Office365/GoDaddy) Setup[/bold]')
        menu_table.add_row('10)', '[bold red]Sentinel Mark X Configuration[/bold red]')
        menu_table.add_row('11)', '[bold cyan]Toggle S/MIME Signing (Verified Sender)[/bold cyan]')
        menu_table.add_row('0)', 'Back to Main Menu')
        ...
        if choice == '1':
            ...
            sentinel_config_menu()
        elif choice == '11':
            cfg = configparser.ConfigParser()
            cfg.read(CONFIG_PATH)
            if not cfg.has_section('SMIME'): cfg.add_section('SMIME')
            
            current = cfg.getboolean('SMIME', 'smime_sign', fallback=False)
            cfg.set('SMIME', 'smime_sign', '0' if current else '1')
            
            with open(CONFIG_PATH, 'w') as f:
                cfg.write(f)
            
            status = "DISABLED" if current else "ENABLED"
            console.print(f"\n[green]S/MIME Signing is now {status}.[/green]")
            if not current:
                 console.print("[yellow]Ensure you have generated certificates using option 3 first![/yellow]")
            time.sleep(2)
        elif choice == '8':
            cfg = configparser.ConfigParser()
            cfg.read(CONFIG_PATH)
            if not cfg.has_section('SMTP'): cfg.add_section('SMTP')
            
            current = cfg.getboolean('SMTP', 'smtp_delete_sent', fallback=False) # type: ignore
            cfg.set('SMTP', 'smtp_delete_sent', '0' if current else '1')
            
            with open(CONFIG_PATH, 'w') as f:
                cfg.write(f)
            
            status = "DISABLED" if current else "ENABLED"
            console.print(f"\n[green]Ghost Sending is now {status}.[/green]")
            console.print("[dim]Note: This works best with Gmail, Outlook/Office365, and Yahoo.[/dim]")
            time.sleep(2)
        elif choice == '0':
            break
        else:
...
def diagnostics_menu():
    ...
    while True:
        ...
        menu_table.add_row('7)', 'Test Zapier Webhook')
        menu_table.add_row('8)', 'View Logs Folder')
        menu_table.add_row('9)', 'Test Groq AI Connection')
        menu_table.add_row('11)', '[bold green]Fetch Dichvusocks Proxies (from Website Scraper)[/bold green]')
        menu_table.add_row('12)', 'List Gemini Models')
        menu_table.add_row('13)', '[bold cyan]Verify Proxy Public IPs[/bold cyan] (Check for leaks)')
        menu_table.add_row('14)', '[bold yellow]Auto-Create GitHub Redirects[/bold yellow] (Batch Generator)')
        menu_table.add_row('15)', '[bold red]Install PDF Engine (Fix PDF Errors)[/bold red]')
        menu_table.add_row('0)', 'Back to Main Menu')
        ...
        if choice == '1':
            ...
            run_script('test_groq.py')
        elif choice == '11':
            run_script('fetch_dichvusocks_proxies.py')
        elif choice == '12':
            run_script('list_gemini_models.py')
        elif choice == '13':
            run_script('verify_proxy_ips.py')
        elif choice == '14':
            run_script('create_github_redirects.py')
        elif choice == '15':
            os.system(os.path.join(PROJECT_ROOT, 'install_pdf_engine.bat'))
            input("\n--- Script finished. Press Enter to return to the menu. ---")
        elif choice == '0':
            break
        else:
...
def toggle_sentinel_mode():
    """Toggles the global Sentinel Mark X engine on or off."""
    console = Console()
    cfg = configparser.ConfigParser()
    cfg.read(CONFIG_PATH)
    ...
    cfg.set('SENTINEL', 'sentinel_enabled', '1' if new_state else '0')
    
    with open(CONFIG_PATH, 'w') as f:
        cfg.write(f)
        
    state_str = "[bold green]ENABLED[/bold green]" if new_state else "[bold red]DISABLED[/bold red]"
    console.print(f"\nSentinel Mark X Defense Engine is now {state_str}.")
    import time
    time.sleep(1.5)
...
def display_status_dashboard():
    ...
    # Sentinel Status
    sentinel_enabled = cfg.getboolean('SENTINEL', 'sentinel_enabled', fallback=False)
    sentinel_status = "[green]ACTIVE[/green]" if sentinel_enabled else "[red]OFF[/red]"
    ...
    features_panel_content = (
        ...
        f"[cyan]Sentinel Defense:[/] {sentinel_status}"
    )
...
def main_menu():
    while True:
        ...
        menu_table.add_row("4) [blue]Data & State Management[/blue]", "5) [bold]Toggle Warm-up Mode[/bold]", "6) [bold red]Toggle Sentinel Defense[/bold red]")
        menu_table.add_row("7) [bold yellow]GitHub Redirects[/bold yellow]", "8) [cyan]Scan & Verify List[/cyan]", "0) [red]Exit[/red]")
...
def verify_list_menu():
    """
    Scan recipients.txt through the Tier 1 list hygiene pipeline and
    write a verified CSV ready for Sentinel Jesko to consume.
    """
    from pathlib import Path
    import configparser
    
    console = Console()
    input_path  = PROJECT_ROOT / "recipients.txt"
    output_path = PROJECT_ROOT / "logs" / "verified_recipients.csv"
    config_path = PROJECT_ROOT / "config" / "config.ini"

    console.print("\n--- [bold cyan]Scan & Verify Recipient List[/bold cyan] ---")

    if not input_path.exists():
        console.print(f"[red]Input file not found:[/red] {input_path}")
        console.print("[yellow]Paste your recipient list into recipients.txt first.[/yellow]")
        input("\n--- Press Enter to return to the menu. ---")
        return

    raw_lines = input_path.read_text(encoding="utf-8").splitlines()
    emails = [ln.strip() for ln in raw_lines if ln.strip() and not ln.strip().startswith("#")]
    if not emails:
        console.print("[yellow]recipients.txt is empty. Nothing to verify.[/yellow]")
        input("\n--- Press Enter to return to the menu. ---")
        return

    console.print(f"Found [bold]{len(emails)}[/bold] lines in recipients.txt.")

    # Offer to update config.ini to point to the fresh CSV
    console.print("\nOptions:")
    console.print("1) Run verification (keep existing recipients_file in config)")
    console.print("2) Run verification AND set config.ini recipients_file to the new CSV")
    console.print("0) Cancel")
    console.print("[yellow]Note: List hygiene runs Tier 1 DNS-only — zero IP risk.[/yellow]")
    ...
    args = ["function/list_hygiene.py", "--input", str(input_path), "--output", str(output_path)]
    run_script("function/list_hygiene.py", args)
    ...
    if sub == '2':
        try:
            ...
            cfg['GENERAL']['recipients_file'] = str(output_path)
            with open(str(config_path), 'w') as f:
                cfg.write(f)
            console.print(f"\n[green]Updated config.ini:[/green] recipients_file = {output_path}")
        except Exception as exc:
            console.print(f"[red]Failed to update config.ini:[/red] {exc}")
